...
function buildSummary(quotes: any[]): string[] {
  ...
  const sp = get("SPX"), nq = get("IXIC"), vix = get("VXX"), tnx = get("TLT"), btc = get("BTC/USD")
  const lines: string[] = []
  if (sp) {
    const p = sp.changePct
    if (p > 0.5)       lines.push(`U.S. stocks are rallying — SPY (S&P 500 proxy) is up ${pct(p)} today, indicating broad market strength.`)
    else if (p < -0.5) lines.push(`U.S. stocks are selling off — SPY (S&P 500 proxy) is down ${pct(p)} today.`)
    else               lines.push(`SPY (S&P 500 ETF) is essentially flat today (${pct(p)}), with no strong directional move.`)
  }
  if (sp && nq) {
    const diff = nq.changePct - sp.changePct
    if (diff > 0.4)       lines.push(`Tech is outperforming — QQQ (Nasdaq-100 ETF) is beating SPY by ${diff.toFixed(1)} percentage points.`)
    else if (diff < -0.4) lines.push(`Tech is lagging today — QQQ (Nasdaq-100 ETF) is trailing SPY by ${Math.abs(diff).toFixed(1)} points.`)
    else                  lines.push(`QQQ and SPY are moving in sync today — no major tech divergence.`)
  }
  if (vix) {
    const v = vix.price
    if (v < 15)      lines.push(`Volatility signals are low (VXX at $${v.toFixed(2)}) — investors appear calm and confident.`)
    else if (v < 20) lines.push(`Volatility signals are moderate (VXX at $${v.toFixed(2)}) — some caution but no panic.`)
    else if (v < 30) lines.push(`Volatility is elevated (VXX at $${v.toFixed(2)}) — expect larger daily swings — expect larger daily swings.`)
    else             lines.push(`Volatility is very high (VXX at $${v.toFixed(2)}) — significant fear in the market — significant fear in the market.`)
  }
  if (tnx) lines.push(`The bond market (TLT) is at $${tnx.price.toFixed(2)}${tnx.changePct < -0.5 ? " — falling, which signals rising yields (bond sell-off)"  // inverted: TLT down = yields up : tnx.changePct < -0.5 ? " and falling, suggesting a flight to safety" : " — stable today"}.`)
  if (btc) {
...
function MarketOverview() {
  ...
  const indices = quotes.filter((q: any) => ["SPX","IXIC","DJI","RUT"].includes(q.symbol))
  const macros  = quotes.filter((q: any) => ["VXX","TLT"].includes(q.symbol))
  const crypto  = quotes.filter((q: any) => ["BTC/USD","ETH/USD"].includes(q.symbol))
  const oil     = quotes.find( (q: any) => q.symbol === "USOIL")
      ...
      {loading && !data ? (
        ...
        <div className="card p-5">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-2">
            {quotes.find((q:any) => q.symbol === "SPX")?.changePct >= 0
            ...
            {macros.map((q: any) => (
              <div key={q.symbol} className="card p-4">
                ...
                <p className="text-lg font-bold font-mono text-slate-100">
                  {q.symbol === "TLT" ? "$" + q.price?.toFixed(2) : q.price?.toFixed(2)}
                </p>
